...
function TestPage() {
    const [testData, setTestData] = useState({
        ...
        totalMarks: 0,
        terminated: false,
        instructions: 'Changing tabs will lead to termination from test' // Hardcoded instructions
    });
    ...
    const duration = 30; // Default duration of the test in minutes
    ...
    console.log("called")
        const response = await fetch('http://localhost:5000/start-test', {
            ...
            body: JSON.stringify({ testId, coursename, studentId })
        });
    ...
    const updateTestStatus = async (recordId, status, marks) => {
    try {
        const response = await fetch(`http://localhost:5000/api/update-test-status/${recordId}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json'
            },
        ...
        const data = await response.json();
        return data; // or handle response data as needed
    } catch (error) {
    ...
    const handleSubmitTest = () => {
        ...
        // Add logic to submit totalMarks to the backend
    };
        ...
        <div className="flex flex-col items-center mt-8">
            ...
            {testData.testStatus !== 'not-started' && (
                <div className="text-center mb-4">
                    <div className="text-lg font-semibold">Time remaining: {Math.floor(testData.remainingTime / 60)} minutes {testData.remainingTime % 60} seconds</div>
